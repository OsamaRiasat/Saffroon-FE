...
import {
  getPMDemandHighestDNo,
  PackingMaterialNames,
  PackingMaterialCodes,
...
class PackingMaterialDemand extends React.Component {
  async componentDidMount() {
    const dm = (await getPMDemandHighestDNo()).data["DNo__max"];
    console.log(dm);
    ...
    const pName = (await PackingMaterialNames()).data; //[{Material:'matname'}]
  ...
  handleSetCatRmcode = async (mat) => {
    const data = (await PackingMaterialSearchByName(mat)).data;
    this.setState((prevState) => ({
      selected: {
        // object that we want to update
        ...prevState.selected, // keep all other key-value pairs
  ...
  handleSetCatMatName = async (code) => {
    const data = (await PackingMaterialSearchByPMCode(code)).data[0];
  ...
  render() {
    ...
    const columns = [
      {
        field: "Category", // binds with object key in list of rows ... name must be same and only one time is bind allow
        headerName: "Category", // optional if not given then fielsd is used as header name by default
        width: 140,
      },
      {
        field: "PMCode",
          ...
          <GridItem xs={12} sm={12} md={12}>
            <Card>
              <CardHeader color="primary">
                <h2>Packing Material Demand Form</h2>
              </CardHeader>
              <CardBody>
                <GridContainer>
                      ...
                      <GridItem xs={12} sm={12} md={12}>
                        ...
                        <GridContainer>
                          <GridItem xs={12} sm={12} md={12}>
                            ...
                            >
                              {this.state.productsPMCode.map((option) => (
                                <MenuItem
                                  key={option.PMCode}
                                  value={option.PMCode}
                                >
                    ...
                    onClick={() => {
                      ...
                      for (let i = 0; i < this.state.cart.length; ++i) {
                        if (
                          ...
                          if (
                            this.state.cart[i].name === this.state.selected.name
                          ) {
                            present = true;
                            break;
                          }
                      ...
                      if (
                        ...
                        console.log("Selected ", this.state.selected.id);
            ...
            <Button
              ...
              onClick={() => {
                ...
                for (let i = 0; i < this.state.cart.length; ++i) {
                  ...
                  if (this.state.cart[i].id === this.state.selectedRows[0]) {
                    index = i;
                    break;
                  }
                }
                ...
                if (index !== -1) {
                  let temp = {
                    ...
                    demandquantity: this.state.cart[index].demandquantity,
