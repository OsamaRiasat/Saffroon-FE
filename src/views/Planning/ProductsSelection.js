...
class ProductsSelection extends React.Component {
  ...
  async componentDidMount() {
    ...
    if (localStorage.getItem("backToPorductSelection")) {
      ...
      const cart_data = (await BackToProductSelection(this.props.planno)).data;
      ...
      console.log("Setting Plan No" + this.state.planno);
    }
  }
  ...
  async saveForm() {
    if (this.state.cart.length !== 0) {
      ...
      if (this.state.first != 0) {
        await updatePostPlan(this.state.planno, payload);
      } else {
  ...
  handleCodeAutofill = async (code) => {
    ...
    if (
      data.data.PackSizesList.length !== 0 &&
      (this.state.dosageType == "Tablet" || this.state.dosageType == "Capsule")
    ) {
  ...
  clearForm = () => {
    ...
    this.setState({ dosageType: "" });
    this.setState({ isfgs: false });
    this.setState({ iswip: false });
  };

  // toggle = () => {
  //   console.log(this.state.dosageType);
                        ...
                        <GridItem xs={12} sm={12} md={3}>
                          <TextField
                            ...
                            onChange={(event) => {
                              ...
                              if (this.state.dosageType != "Vial") {
                                let packSizeArr =
                                  this.state.selected.packsize.split("x");
                                packSize =
                              ...
                              this.setState((prevState) => ({
                                selected: {
                                  ...prevState.selected,
                                  batches: (
                                    this.state.units / this.state.standardUnits
                                  ).toFixed(0),
                        ...
                        <GridItem xs={12} sm={12} md={3}>
                          <TextField
                            ...
                            onChange={(event) => {
                              ...
                              this.setState({
                                units:
                                  event.target.value * this.state.standardUnits,
                              });
                              let packSize;
                              if (this.state.dosageType != "Vial") {
                                let packSizeArr =
                        ...
                        <GridItem xs={12} sm={12} md={3}>
                          <TextField
                            ...
                            onChange={(event) => {
                              ...
                              this.setState({
                                previousUnit: event.target.value,
                              });
                              let packSize;
                              if (this.state.dosageType != "Vial") {
                                let packSizeArr =
