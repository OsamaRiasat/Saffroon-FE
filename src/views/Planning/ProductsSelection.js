...
class ProductsSelection extends React.Component {
  async componentDidMount() {
    ...
    this.props.handle_plan(dm + 1);
      ...
      const cart_data = (await BackToProductSelection(this.props.planno-1)).data;
  ...
  handleCodeAutofill = async (code) => {
    ...
    if (data.data.PackSizesList.length !== 0) {
      let packs_temp = parseInt(
        data.data.PackSizesList[0].substring(
          data.data.PackSizesList[0].indexOf("x") + 1
        )
      );
      packs_temp = (data.data.units / packs_temp).toFixed(0);
      this.setState((prevState) => ({
        selected: {
          // object that we want to update
          ...prevState.selected, // keep all other key-value pairs
                      ...
                      <GridContainer>
                        <GridItem xs={12} sm={12} md={4}>
                          <TextField
                            ...
                            label="Product Code:"
                            fullWidth="true"
                            value={this.state.selected.pcode}
                            onChange={(event) => {
                              this.setState((prevState) => ({
                                selected: {
                                  // object that we want to update
                                  ...prevState.selected, // keep all other key-value pairs
                                  pcode: event.target.value, // update the value of specific key
                                },
                              }));

                              this.handleCodeAutofill(event.target.value);
                            }}
                          >
                            {this.state.pcode_arr.map((pc) => (
                              <MenuItem
                                key={pc.ProductCode}
                        ...
                        <GridItem xs={12} sm={12} md={4}>
                          <TextField
                            ...
                            label="Product:"
                            fullWidth="true"
                            value={this.state.selected.pname}
                            onChange={(event) => {
                              this.setState((prevState) => ({
                                selected: {
                                  // object that we want to update
                                  ...prevState.selected, // keep all other key-value pairs
                                  pname: event.target.value, // update the value of specific key
                                },
                              }));

                              this.handleNameAutofill(event.target.value);
                            }}
                          >
                            {this.state.pname_arr.map((pn) => (
                              <MenuItem key={pn.Product} value={pn.Product}>
                                {pn.Product}
                              </MenuItem>
                            ))}
                          </TextField>
                        ...
                        <GridItem xs={12} sm={12} md={4}>
                          ...
                          >
                            {this.state.packsize_arr.map((ps) => (
                            ...
                            onChange={(event) => {
                              this.setState((prevState) => ({
                                selected: {
                                  // object that we want to update
                                  ...prevState.selected, // keep all other key-value pairs
                                  packs: event.target.value, // update the value of specific key
                                },
                              }));
                            }}
                          />
                            ...
                            onChange={(event) => {
                              this.setState((prevState) => ({
                                selected: {
                                  // object that we want to update
                                  ...prevState.selected, // keep all other key-value pairs
                                  batches: event.target.value, // update the value of specific key
                                },
                              }));

                              let tempStandardUnits = (
                                this.state.standardUnits /
                                this.state.standardBatch
                              ).toFixed(0);
                              this.setState({
                                units: event.target.value * tempStandardUnits,
                              });
                              let packs_temp = parseInt(
                                this.state.selected.packsize.substring(
                                  this.state.selected.packsize.indexOf("x") + 1
                                )
                              );
                              packs_temp = (
                                (event.target.value * tempStandardUnits) /
                                packs_temp
                              ).toFixed(0);
                              this.setState(
                                (prevState) => ({
                                  selected: {
                                    // object that we want to update
                                    ...prevState.selected, // keep all other key-value pairs
                                    packs: packs_temp, // update the value of specific key
                                  },
                                }),
                                // () => {
                                //   this.setCart();
                            ...
                            onChange={(event) => {
                              ...
                              this.setState((prevState) => ({
                                selected: {
                                  ...
                                  ...prevState.selected, // keep all other key-value pairs
                                  batches: Math.ceil(
                                    event.target.value * tempStandardBatches
                        ...
                        <GridItem xs={12} sm={12} md={6}>
                          <Button
                            ...
                            onClick={() => {
                              ...
                              // pcode: "",
                              // packs: "",
                              if(this.state.selected.batches==="" || this.state.selected.pcode==="" || this.state.selected.packs==="" || this.state.selected.packsize==="" || this.state.selected.pname==="" || this.state.fgs_qty===""|| this.state.wip_qty==="" )
                              {
